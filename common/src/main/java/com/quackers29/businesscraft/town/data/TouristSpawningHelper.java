 ...
 * destination selection, spawn position calculation, and tourist entity
 * creation.
 */
public class TouristSpawningHelper {
    ...
    public boolean spawnTouristOnPlatform(Level level, Town town, Platform platform, UUID originTownId) {
        ...
        if (!town.canAddMoreTourists()) {
            DebugConfig.debug(LOGGER, DebugConfig.TOURIST_SPAWNING,
                    "Cannot spawn tourist - town {} at maximum capacity ({}/{})",
    ...
    /**
     * Selects a destination town for a tourist based on platform configuration and
     * town population
     * 
     * @param level        The level
     * @param platform     The platform the tourist is spawning from
     * @param originTownId The ID of the origin town
     * @return The selected destination town ID, or null if no valid destination
     *         found
     */
     ...
     * @param allowedTowns List of allowed town IDs, or null for all towns except
     *                     current
    ...
    private boolean spawnTourist(Level level, BlockPos pos, Town originTown, Platform platform,
        ...
        if (level.getBlockState(pos).isAir() && level.getBlockState(pos.above()).isAir()) {
            ...
            TouristEntity tourist = new TouristEntity(
                    (net.minecraft.world.entity.EntityType<TouristEntity>) PlatformAccess.getEntities()
                            .getTouristEntityType(),
                    level,
                    originTown,
                    platform,
                    destinationTownId,
