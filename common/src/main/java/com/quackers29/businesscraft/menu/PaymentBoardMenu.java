...
public class PaymentBoardMenu extends AbstractContainerMenu {
    ...
    public void claimReward(UUID rewardId, boolean toBuffer) {
        if (townBlockPos != null) {
            ...
            try {
                PlatformAccess.getNetworkMessages().sendToServer(new com.quackers29.businesscraft.network.packets.storage.PaymentBoardClaimPacket(
    ...
    public boolean processBufferStorageAdd(Player player, int slotId, ItemStack itemStack) {
        ...
        try {
            PlatformAccess.getNetworkMessages().sendToServer(new com.quackers29.businesscraft.network.packets.storage.BufferStoragePacket(
