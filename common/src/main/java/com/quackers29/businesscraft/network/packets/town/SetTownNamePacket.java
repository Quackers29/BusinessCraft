        ...
        PlatformAccess.getNetwork().enqueueWork(context, () -> {
            handlePacket(context, (player, townBlock) -> {
                DebugConfig.debug(LOGGER, DebugConfig.NETWORK_PACKETS,
                        "Processing SetTownNamePacket for position {} with new name: '{}'",
                        pos, newName);
                ...
                if (provider != null && provider instanceof Town town) {
                    ...
                    if (trimmedName.isEmpty()) {
                        player.sendSystemMessage(
                                Component.literal("Town name cannot be empty").withStyle(ChatFormatting.RED));
                        return;
                    }

                    if (trimmedName.length() > 30) {
                        player.sendSystemMessage(Component.literal("Town name cannot exceed 30 characters")
                    ...
                    DebugConfig.debug(LOGGER, DebugConfig.NETWORK_PACKETS,
                            "Cleared ClientTownMapCache after town rename");

                    // Invalidate TownDataCache in the current screen to refresh the overview tab
                    try {
                        com.quackers29.businesscraft.api.ClientHelper clientHelper = PlatformAccess.getClient();
                        if (clientHelper != null) {
                            clientHelper.invalidateTownScreenCache();
                            DebugConfig.debug(LOGGER, DebugConfig.NETWORK_PACKETS,
                                    "Invalidated TownDataCache for immediate UI refresh");
                        }
                    } catch (Exception e) {
                        DebugConfig.debug(LOGGER, DebugConfig.NETWORK_PACKETS, "Could not invalidate screen cache: {}",
                                e.getMessage());
                    }
