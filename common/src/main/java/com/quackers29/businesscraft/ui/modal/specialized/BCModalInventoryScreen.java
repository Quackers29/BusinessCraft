 ...
 *             Use BCModalInventoryScreenV2 instead, which provides the same
 *             functionality
 ...
 *             - StorageOperationsManager: Manages storage-specific
 *             functionality
...
public class BCModalInventoryScreen<T extends AbstractContainerMenu> extends AbstractContainerScreen<T> {
    ...
    // Display title that can be updated (unlike the final 'title' field inherited
    // from Screen)
    ...
    public BCModalInventoryScreen(
        ...
        // Move the title and inventory label off-screen to hide them (we'll draw our
        // own)
    ...
    private void renderTradeOutputSection(GuiGraphics guiGraphics, int mouseX, int mouseY) {
        ...
        String hintText = "Currency";
        TradingViewModel vm = getTradingViewModel();
        if (vm != null) {
            hintText = vm.getCurrencyName();
        }
        int hintWidth = this.font.width(hintText);
        ...
        if (hasItemsToTrade) {
            ...
            if (vm != null) {
                ...
                if (type != null) {
                    ...
                    if (info != null) {
                        float totalValue = itemCount * info.pricePerUnit();
                        earnings = (int) totalValue;
                    }
                } else {
                    // Fallback for unregistered items (shouldn't really happen if filter works)
                    // or maybe default logic
                    earnings = itemCount; // 1:1 fallback?
                }
            } else {
                // Fallback to legacy if no VM
            ...
            String rateText = String.format("%d %s = %d %s",
                    itemCount, shortName, earnings, currencyName);
    ...
    private void renderStorageElements(GuiGraphics guiGraphics, int mouseX, int mouseY) {
        // The storage slots are already handled by the AbstractContainerScreen's
        // rendering
    }
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        if (button == 0 && this.inventoryType == InventoryType.TRADE
                && isMouseOverTradeButton((int) mouseX, (int) mouseY)) {
            // Process the trade
            processTrade();
        ...
        if (this.minecraft != null && this.minecraft.player != null) {
            ...
            try {
                ...
                if (townPos != null) {
                    ...
                    DebugConfig.debug(LOGGER, DebugConfig.STORAGE_OPERATIONS,
                            "Requesting personal storage data for player {} at position {}", playerId, townPos);
                    PlatformAccess.getNetworkMessages()
                            .sendToServer(new PersonalStorageRequestPacket(townPos, playerId));
                    ...
                    if (townMenu != null) {
                        ...
                        DebugConfig.debug(LOGGER, DebugConfig.STORAGE_OPERATIONS,
                                "Updated personal storage display with {} items from local cache",
                                personalItems.size());
                    } else {
    ...
    protected void slotClicked(Slot slot, int slotId, int mouseButton, ClickType type) {
            ...
            if (itemsCollected > 0) {
                ...
                if (townPos != null && this.minecraft != null && this.minecraft.player != null) {
                    ...
                    if (isPersonalMode) {
                        ...
                        PlatformAccess.getNetworkMessages()
                                .sendToServer(new CommunalStoragePacket(townPos, ItemStack.EMPTY, -1, true));
                    }
                }
            }
                    ...
                    for (int affectedSlotId : affectedDragSlots) {
                        ...
                        if (affectedSlot != null) {
                            ...
                            if (!slotStack.isEmpty()) {
                                ...
                                if (isPersonalMode) {
                                    storageMenu.processPersonalStorageAdd(this.minecraft.player, affectedSlotId,
                                            slotStack.copy());
                                } else {
                                    storageMenu.processCommunalStorageAdd(this.minecraft.player, affectedSlotId,
                                            slotStack.copy());
                                }
                                anySlotUpdated = true;
                            }
                    ...
                    if (!anySlotUpdated && !affectedDragSlots.isEmpty()) {
                        if (isPersonalMode) {
                            ...
                            PlatformAccess.getNetworkMessages().sendToServer(
                                    new PersonalStorageRequestPacket(townPos, this.minecraft.player.getUUID()));
                        } else {
                            // Request communal storage data refresh
                            PlatformAccess.getNetworkMessages()
                                    .sendToServer(new CommunalStoragePacket(townPos, ItemStack.EMPTY, -1, true));
            ...
            if (type == ClickType.PICKUP && mouseButton == 0 && !this.menu.getCarried().isEmpty()) {
                ...
                if (slotAfter.getCount() > slotBefore.getCount() ||
                    ...
                    if (itemsAdded > 0) {
                        ...
                        DebugConfig.debug(LOGGER, DebugConfig.STORAGE_OPERATIONS,
                                "Adding {} items to storage at slot {}", itemsAdded, slotId);
                ...
                // Handle the case where we swapped different items (not just adding more of the
                // same type)
        ...
        if (type == ClickType.QUICK_MOVE && slot != null && storageMenu != null) {
            ...
            if (isStorageSlot && !slotBefore.isEmpty()
                    && (slotAfter.isEmpty() || slotAfter.getCount() < slotBefore.getCount())) {
                ...
                for (int i = 0; i < 18; i++) {
                    ...
                    if (!storageSlotBefore.isEmpty() && !storageSlotAfter.isEmpty() &&
                        ...
                        if (storageSlotAfter.getCount() > storageSlotBefore.getCount()) {
                            ...
                            // Special case: Same count but potentially a new item instance replacing the
                            // old one
                    ...
                    // Case 2: New stack created in empty slot
                        ...
                        if (isMatchingItem) {
                            // Process the storage add operation based on mode
                            if (isPersonalMode) {
                                storageMenu.processPersonalStorageAdd(this.minecraft.player, i,
                                        storageSlotAfter.copy());
                            } else {
                                storageMenu.processCommunalStorageAdd(this.minecraft.player, i,
                                        storageSlotAfter.copy());
                            }
                            anySlotUpdated = true;
                        } else {
                            // This slot changed but doesn't match our shift-clicked item - this shouldn't
                            // happen
