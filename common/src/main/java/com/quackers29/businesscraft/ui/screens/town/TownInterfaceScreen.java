...
import com.quackers29.businesscraft.ui.tabs.*;
 ...
 * This screen demonstrates various UI components and layouts using the enhanced
 * BCTabPanel.
 * Refactored to use dependency injection and extracted managers for better
...
public class TownInterfaceScreen extends BaseTownScreen<TownInterfaceMenu>
    ...
    public TownInterfaceScreen(TownInterfaceMenu menu, Inventory inventory, Component title) {
        ...
        DebugConfig.debug(LOGGER, DebugConfig.UI_BASE_SCREEN, "TownInterfaceScreen created with radius: {}",
                currentSearchRadius);
    }
    ...
    protected void initializeSpecificManagers() {
        try {
            ...
            DebugConfig.debug(LOGGER, DebugConfig.UI_BASE_SCREEN,
                    "TownInterfaceScreen managers initialized successfully");
        } catch (Exception e) {
    ...
    @Override
    public void onContractBoard() {
        if (dependencies != null) {
            dependencies.getButtonCoordinator().handleContractBoard();
        } else {
            LOGGER.warn("Dependencies not available for onContractBoard");
        }
    }
    ...
    public void handleRadiusChange(int mouseButton) {
        ...
        try {
            ...
            DebugConfig.debug(LOGGER, DebugConfig.SEARCH_RADIUS_MANAGER, "Search radius changed to: {}",
                    result.getNewRadius());
    ...
    // These methods are kept for backwards compatibility but delegate to
    // coordinators
     ...
     * Called when town data changes (e.g., after renaming) to ensure UI shows
     * updated information.
     */
