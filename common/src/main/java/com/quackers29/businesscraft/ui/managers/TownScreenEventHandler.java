...
public class TownScreenEventHandler {
    ...
    public boolean handleMouseScrolled(double mouseX, double mouseY, double delta,
        ...
        // Get active tab and forward scroll events to all scrollable tabs in the same
        // way
        BCTabPanel tabPanel = stateProvider.getTabPanel();
        if (tabPanel != null) {
            String activeTabId = tabPanel.getActiveTabId();

            // Forward scroll events to the children of the active tab
            // This allows specific components inside the tab (like StandardTabContent) to
            // handle scrolling
            // regardless of the tab's ID
            BCPanel panel = tabPanel.getTabPanel(activeTabId);
            if (panel != null) {
    ...
    public boolean handleKeyPressed(int keyCode, int scanCode, int modifiers,
        ...
        if (activePopup != null && activePopup.isVisible()) {
            ...
            // For string input popups, completely consume 'e' key (inventory key) to
            // prevent it from closing the popup
            ...
            // If it's an input popup, consume all uncaught key events to prevent them from
