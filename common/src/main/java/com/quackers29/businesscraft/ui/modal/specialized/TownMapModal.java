...
public class TownMapModal extends Screen {
     ...
     * @param onCloseCallback Optional callback to execute when closing (can be
     *                        null)
     */
    public TownMapModal(Screen parentScreen, BlockPos currentTownPos, Consumer<TownMapModal> onCloseCallback) {
        ...
        DebugConfig.debug(LOGGER, DebugConfig.UI_MANAGERS, "TownMapModal created with current town at: {}",
                currentTownPos);
    }
    ...
    /**
     * Check if a town is within the current view (always true now for edge
     * indicators)
     */
    ...
    private void drawTown(GuiGraphics guiGraphics, TownMapDataResponsePacket.TownMapInfo town, int mouseX, int mouseY) {
        ...
        int screenX, screenY;
        // Current town should NEVER be clamped to edges - always show at actual
        // position
        ...
        // For edge indicators (non-current towns outside bounds), make them dimmer and
        // triangular
        if (!withinBounds && !isCurrentTown) {
    ...
    private void drawSelectedTownPlatforms(GuiGraphics guiGraphics) {
        ...
        for (TownPlatformDataResponsePacket.PlatformInfo platform : selectedTownPlatforms.values()) {
            // Skip if positions are invalid (e.g. newly created platform)
            if (platform.startPos == null || platform.endPos == null) {
                continue;
            }
            ...
            if (isLineVisible(startScreenX, startScreenY, endScreenX, endScreenY, mapLeft, mapTop, mapRight,
                    mapBottom)) {
                ...
                int markerSize = Math.max(1, (int) (2 * Math.min(zoomLevel / 10.0, 2.0))); // Size scales with zoom, 50%
                                                                                           // smaller
                ...
                if (isPointInBounds(startScreenX, startScreenY, mapLeft, mapTop, mapRight, mapBottom)) {
                    ...
                    if (isSelected) {
                        ...
                        guiGraphics.hLine(startScreenX - ringSize, startScreenX + ringSize, startScreenY - ringSize,
                                0xFFFFFFFF);
                        guiGraphics.hLine(startScreenX - ringSize, startScreenX + ringSize, startScreenY + ringSize,
                                0xFFFFFFFF);
                        guiGraphics.vLine(startScreenX - ringSize, startScreenY - ringSize, startScreenY + ringSize,
                                0xFFFFFFFF);
                        guiGraphics.vLine(startScreenX + ringSize, startScreenY - ringSize, startScreenY + ringSize,
                                0xFFFFFFFF);
                    }
                ...
                if (isPointInBounds(endScreenX, endScreenY, mapLeft, mapTop, mapRight, mapBottom)) {
                    ...
                    if (isSelected) {
                        int ringSize = markerSize + 2;
                        guiGraphics.hLine(endScreenX - ringSize, endScreenX + ringSize, endScreenY - ringSize,
                                0xFFFFFFFF);
                        guiGraphics.hLine(endScreenX - ringSize, endScreenX + ringSize, endScreenY + ringSize,
                                0xFFFFFFFF);
                        guiGraphics.vLine(endScreenX - ringSize, endScreenY - ringSize, endScreenY + ringSize,
                                0xFFFFFFFF);
                        guiGraphics.vLine(endScreenX + ringSize, endScreenY - ringSize, endScreenY + ringSize,
                                0xFFFFFFFF);
                    }
                ...
                if (zoomLevel >= 5.0
                        && isPointInBounds(startScreenX, startScreenY, mapLeft, mapTop, mapRight, mapBottom)) {
    ...
    private void drawSelectedTownBoundary(GuiGraphics guiGraphics) {
        ...
        // Draw the circular boundary using line segments (64 segments for smooth
        // circle)
        int segments = 64;
     ...
     * Returns clipped line coordinates [x1, y1, x2, y2] or null if completely
     * outside
     */
    ...
    private void drawTownInfo(GuiGraphics guiGraphics) {
        if (selectedTown == null || currentTownPos == null)
            return;
        ...
        if (selectedPlatform != null) {
            ...
            guiGraphics.drawString(this.font, "Selected: " + selectedPlatform.name + " " + statusText, infoX + 5,
                    infoY + 8, TEXT_COLOR);
            ...
            guiGraphics.drawString(this.font, String.format("Start Pos: %d, %d, %d",
                    selectedPlatform.startPos.getX(), selectedPlatform.startPos.getY(),
                    selectedPlatform.startPos.getZ()),
            ...
            guiGraphics.drawString(this.font, String.format("Distance: %.1f blocks", distance), infoX + 5, infoY + 20,
                    TEXT_COLOR);
            guiGraphics.drawString(this.font,
                    String.format("Position: X %d, Z %d", selectedPos.getX(), selectedPos.getZ()), infoX + 5,
                    infoY + 32, TEXT_COLOR);
            guiGraphics.drawString(this.font, "Population: " + selectedTown.population, infoX + 5, infoY + 44,
                    TEXT_COLOR);
        }
    }
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        if (button == 0) { // Left click
            // Check if clicking on a platform first (if we have a selected town with
            // platforms)
            if (selectedTown != null && selectedTownPlatforms != null) {
                TownPlatformDataResponsePacket.PlatformInfo clickedPlatform = getPlatformAtPosition((int) mouseX,
                        (int) mouseY);
    ...
    private TownPlatformDataResponsePacket.PlatformInfo getPlatformAtPosition(int screenX, int screenY) {
        if (selectedTownPlatforms == null || selectedTownPlatforms.isEmpty())
            return null;
    ...
    public void refreshFromCache() {
        ...
        if (cache.hasData()) {
            ...
            DebugConfig.debug(LOGGER, DebugConfig.UI_MANAGERS, "Town data refreshed from cache: {} towns",
                    this.allTowns.size());
        }
    }
    ...
    /**
     * Handle town data response from server (platform packet context - includes
     * live boundary)
     */
    public void refreshTownData(UUID townId, TownPlatformDataResponsePacket.TownInfo townInfo) {
        if (selectedTown != null && selectedTown.id.equals(townId)) {
            ...
            // If this town data is for a different town than selected, clear platform
            // selection
            if (selectedPlatform != null) {
                selectedPlatform = null;
                DebugConfig.debug(LOGGER, DebugConfig.UI_MANAGERS,
