...
2026-06-11 | T-033 | DONE | ClientSyncHelper name resolution (client cache + short fallback), cache updates, visit load side-effects, resource tag roundtrips (registry keys) + 15 tests (McBootstrap + doubles); server TownManager path left NEEDS-MC; full suite green; docs first.
2026-06-11 | T-034 | NEEDS-MC | TownContractComponent autonomous excess sell creation + need-based bidding documented (excessRatio/needRatio/bidModifier/courier budget/targetStock/mutual-exclusion/3-cap/randomness + worked examples); 4 smoke + NBT persistence tests; core requires live managers/Level/registry so NEEDS-MC; full suite green.
2026-06-11 | T-035 | DONE | Work unit add/set math (addExact+overflow-to-MAX/0+0-floor+optional cap) documented + 20 tests; set-bypasses-cap quirk pinned; full suite green; docs first.
2026-06-11 | T-036 | DONE | Boundary geometry on Town (50 fallback, 3D distSqr<=r*r inside, <sum-r overlap, r1+r2 min) documented + 17 tests (extend TownTest); pop-vs-border + service dupe quirks pinned; full suite green; docs first.
