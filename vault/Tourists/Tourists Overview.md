...
- **[[Tourists/Spawning/Tourist Spawning and Destination Selection|Tourist Spawning and Destination Selection]]** (T-037) — when a town passes capacity checks, the helper places a TouristEntity at a random point along a platform's path (tries three times to avoid crowding); the tourist's destination is either one of the platform's explicitly enabled target towns (chosen fairly by the allocation tracker using live populations) or the special "Any Town" sentinel. Expiry is set from config at spawn; the origin town's tourist counter is incremented on success.
- **[[Tourists/Lifecycle/Tourist Distance Tracking, Ride Extension and Expiry|Tourist Distance Tracking, Ride Extension and Expiry]]** (T-038) — tourists accumulate real distance traveled from their own position samples (the value that actually pays destinations); their lifetime only counts down while they are stationary, extends once when they board minecarts or Create carriages, and their appearance (skin tier) and trading UI progress bar advance with cumulative meters.
