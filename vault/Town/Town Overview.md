...
- **[[Town/Data Synchronization/Container Data Registration|Container Data Registration]]** (T-024) — The Town Interface menu stays in sync with live town values (population, spawn flag, radius, tourist stats) through a named-field ContainerData system. Fields are registered with live getters/setters; the helper manages Minecraft's index-based sync, dirty tracking, and read-only protection so the UI always reflects current state without hard-coded slots.
- **[[Town/Data Synchronization/Client Data Synchronization|Client Data Synchronization]]** (T-033) — Client UI caches (resources, escrow, wanted, personal, visit history) are populated from server NBT tags using registry-stable keys; town names for visit records are pre-resolved on server or use short-ID fallback so lists and tooltips never block on lookups.
- **[[Town/Production/Recipe Execution and Dynamic Evaluation|Recipe Execution and Dynamic Evaluation]]** (T-029) — Production recipes (population growth, work units, farming, tourist generation) run automatically on a progress clock; amounts scale with pop/happiness/wu or upgrade modifiers, conditions gate on stock thresholds or surplus, outputs can stall at caps, and food shortages trigger starvation penalties on the maintenance recipe.
