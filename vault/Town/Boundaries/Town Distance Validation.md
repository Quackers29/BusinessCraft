...
- Test files:
  ...
  - `common/src/test/java/com/quackers29/businesscraft/town/service/TownBoundaryServiceTest.java` (5 guard tests)
- Covered (pure, no MC bootstrap):
  - TownValidationService: all name validation paths (null/empty/len/regex/inappropriate via reflection on private containsInappropriateContent), position (BlockPos Y/X/Z bounds + null), search radius, tourist management (negatives + insufficient pop vs Config.minPopForTourists), visitor processing, creation/settings orchestration (empty resources path), null-item resource guards.
  - TownBoundaryService: null-town calculateBoundaryRadius, both-null getMinimumDistanceRequired, null-pos and no-existing-towns checkTownPlacement, null-town checkBoundaryExpansion. (These exercise the service class, Result/BCError, and Config default path.)
- Not unit-tested (NEEDS-MC): the core distance math + conflict detection in checkTownPlacement / checkBoundaryExpansion / getMinimum... with real Town instances (new Town(...) triggers ExceptionInInitializerError from component/UpgradeRegistry/Platform static init even with registry stub — identical limitation as T-007 and T-002). The full formulas, worked examples, and edge cases (including the pop-vs-border discrepancy) are extracted from source and recorded in this note instead.
- Config save/restore: defaultStartingPopulation + minPopForTourists. Registry stub installed in @BeforeEach (TestRegistryHelper) per established pattern.
- Reflection: only for the private containsInappropriateContent in ValidationService.
