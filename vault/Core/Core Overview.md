...
- **[[Core/Util/Error Types|Error Types]]** (T-026) — categorized failure reasons (validation, town rules, UI, network, data saves, config, unexpected) with simple factory helpers that produce a stable code plus a readable message. These objects are the conventional error side of Result returns so every caller sees exactly why an operation was rejected.
