...
public class FabricRenderHelper implements RenderHelper {
    ...
    public void renderOverlay(Object guiGraphics, float partialTick, int screenWidth, int screenHeight) {
        // This method is not used directly - overlays are registered with Fabric's
        // InGameHud
    ...
    public Object getPoseStack(Object renderEvent) {
        try {
            ...
            Object result = getMatrixStackMethod.invoke(renderEvent);
            return result;
        } catch (Exception e) {
    ...
    public boolean isRenderStage(Object renderEvent, String stageName) {
        // For Fabric, we register callbacks for specific render events (e.g.,
        // AFTER_TRANSLUCENT)
        // So if this callback is being invoked, we're already at the correct stage
        // Always return true since the event registration handles stage filtering
        return true;
    }
    ...
    private void registerOverlayWithFabric(String overlayId, OverlayRenderer overlay) {
        ...
        try {
            ...
            Class<?> hudRenderCallbackClass = classLoader
                    .loadClass("net.fabricmc.fabric.api.client.rendering.v1.HudRenderCallback");

            // Register the overlay callback
            java.lang.reflect.Method registerMethod = hudRenderCallbackClass.getMethod("register",
                    ...
                    (proxy, method, args) -> {
                        if (method.getName().equals("onHudRender")) {
                            ...
                            if (minecraft != null) {
                                ...
                                java.lang.reflect.Method getGuiScaledWidthMethod = window.getClass()
                                        .getMethod("getGuiScaledWidth");
                                java.lang.reflect.Method getGuiScaledHeightMethod = window.getClass()
                                        .getMethod("getGuiScaledHeight");
                                ...
                                // Cast guiGraphics and call overlay renderer directly (OverlayRenderer now
                                // accepts Object)
    ...
    /**
     * Unregister overlay from Fabric (note: Fabric doesn't have direct unregister,
     * but we track it)
     */
    ...
    /**
     * Register world render callback with Fabric's WorldRenderEvents using
     * reflection
     */
    private void registerWorldRenderCallbackWithFabric(String renderStage, WorldRenderCallback callback) {
        try {
            ClassLoader classLoader = FabricRenderHelper.class.getClassLoader();
            Class<?> worldRenderEventsClass = classLoader
                    .loadClass("net.fabricmc.fabric.api.client.rendering.v1.WorldRenderEvents");
            ...
            Class<?> worldRenderContextClass = classLoader
                    .loadClass("net.fabricmc.fabric.api.client.rendering.v1.WorldRenderContext");
